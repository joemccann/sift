...
APP_EXECUTABLE="$APP_PATH/Contents/MacOS/SiftApp"
TMP_DIR="$(mktemp -d)"
...
/usr/bin/pkill -x SiftApp >/dev/null 2>&1 || true
for _ in {1..50}; do
  if ! /usr/bin/pgrep -x SiftApp >/dev/null 2>&1; then
    break
  fi
  /bin/sleep 0.2
done
...
/usr/bin/osascript -e 'tell application id "local.sift.macos" to activate' >/dev/null 2>&1 || true
