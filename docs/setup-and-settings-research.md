...
This note captures the current insertion points and persistence seams for first-run setup and persistent settings in the Sift package.

## Current State Ownership

- `SiftApp` owns a single `SiftViewModel` for the app window.
- `SiftViewModel` owns the runtime state for:
  - `settings`
  - `sources`
  - `selectedSource`
  - `transcript`
...
- `SiftApp` decides whether to show `SetupFlowView` or `SiftRootView`.
- `SiftViewModel.requiresInitialSetup` drives that decision.
...
```text
~/Library/Application Support/Sift/session.json
```
...
- `Sources/SiftApp/SiftApp.swift`
- `Sources/SiftKit/SiftViewModel.swift`
- `Sources/SiftKit/SetupFlowView.swift`
- `Sources/SiftKit/AppSessionStore.swift`
- `Sources/SiftKit/KeychainStore.swift`
- `Sources/SiftCore/AppSettings.swift`
